...
Frontend	Vanilla HTML/CSS/JS, QuaggaJS for barcode detection
...
ML layer (classifier/model.py) — a Random Forest classifier trained on ~400,000 real products from the Open Food Facts bulk data export, using Nutri-Score A/B vs D/E as ground-truth labels. 98% accuracy, 97–99% precision/recall across both classes on a held-out test set. See Limitations for an important caveat on that number.
...
Download the Open Food Facts CSV export from world.openfoodfacts.org/data
Place it in data/
Update CSV_PATH and OUTPUT_PATH in data/train_model.py if needed
Run python train_model.py from inside data/
API
